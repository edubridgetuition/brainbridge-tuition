        ...
        <div className="receipt-print" style={{ padding: '2.5rem', fontFamily: "'Inter', sans-serif" }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', borderBottom: '2px solid #e2e8f0', paddingBottom: '1.5rem' }}>
            <div>
              <div style={{ display: 'flex', alignItems: 'center', gap: '0.75rem', color: '#6366f1' }}>
                {tenantLogo ? (
                  <img src={tenantLogo} alt="Logo" style={{ height: '36px', width: 'auto', objectFit: 'contain' }} />
                ) : (
                  <GraduationCap size={32} />
                )}
                <span style={{ fontSize: '1.75rem', fontWeight: '800', tracking: '-0.02em', color: '#0f172a' }}>
              ...
              <p style={{ fontSize: '0.85rem', color: '#64748b', marginTop: '0.5rem', lineHeight: '1.4', whiteSpace: 'pre-line' }}>
                {receiptSubHeader ? receiptSubHeader : (
                  <>
                    Official tuition fees receipt generated by {tenantName || 'EduBridge'}.<br />
                    {tenantWhatsapp ? `Tel: +91 ${tenantWhatsapp}` : 'Tel: +91 98765 43210'} | Email: support@{(tenantName || 'EduBridge').toLowerCase().replace(/\s+/g, '')}.com
                  </>
                )}
              </p>
            </div>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', marginTop: '3rem', borderTop: '1px dashed #e2e8f0', paddingTop: '1.5rem' }}>
            <div style={{ fontSize: '0.75rem', color: '#94a3b8', maxWidth: '350px', lineHeight: '1.4' }}>
              {receiptFooterNote1 ? (
                <div>{receiptFooterNote1}</div>
              ) : (
                <div>* This is a computer-generated invoice and does not require a physical signature.</div>
              )}
              {receiptFooterNote2 ? (
                <div>{receiptFooterNote2}</div>
              ) : (
                <div>* Thank you for choosing {tenantName || 'EduBridge Academy'}!</div>
              )}
            </div>
